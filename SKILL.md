...
### MANDATORY: Final Summary Table (IN CONVERSATION, NOT JUST IN FILE)

**Every task MUST end by running `cite_table.py` and showing its output directly in the conversation. This is non-negotiable.**
...
Then relay the table to the user in plain text — this is mandatory:
1. `Read: scripts/cite_output.txt` to get the table content
2. **Paste the table directly in your reply message.** Your own message text never gets collapsed by the UI.

The table MUST appear as readable text in the conversation, not hidden behind a collapsed tool output. The disclaimer ("This table was generated by a hard-coded script...") must also be relayed.

If the user also requested a file, write the file too.
