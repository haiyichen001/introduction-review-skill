...
Then **immediately Read the output file** and display its contents as a regular message:
```
Read: scripts/cite_output.txt
```

The script writes the table to both stdout AND `scripts/cite_output.txt`. Reading the file avoids Claude Code's tool-output collapse — the table appears directly in the conversation.

If the user also requested a file, write it too — but this relay step is mandatory regardless.
