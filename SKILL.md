...
The 4-column table (# | Author | Context | Status) is the skill's final report. It proves every citation is real and traceable. The disclaimer line ("This table was generated by a hard-coded script...") must also be visible.

If the user also requested a file, write the file too — but the table output in the conversation is mandatory regardless.
